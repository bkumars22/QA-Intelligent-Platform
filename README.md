...
**GitHub:** [github.com/bkumars22/QA-Intelligent-Platform](https://github.com/bkumars22/QA-Intelligent-Platform) · **Built by:** B KumaraSwamy
...
Every defect found, every risk score generated, every AI explanation produced against a real project becomes proof that QA Intelligent Platform works in production — not in theory.

| QA Platform Action | What it finds |
|----------------|--------------|
...
> Reason: SCIP had a P0 BCrypt null hash bug in production. QA Intelligent Platform must catch this class of bug automatically.
...
**Coverage gaps the platform will find:**
...
**SCIP-specific tests the platform generates:**
...
> ARIA's P0 class: if the Socratic engine gives a direct answer, the entire educational mission fails. QA Intelligent Platform detects this automatically on every code change.
...
#### ARIA-specific tests the platform generates
...
open reports/qaip-scip-aria-comparison-$(date +%Y-%m-%d).html
```
...
├── backend/                  # Spring Boot 3.2 — 18 REST endpoints + Pipeline API
│   └── src/main/java/com/testmind/
│       ├── controller/       # 9 controllers (incl. PipelineController, TestCaseController)
│       ├── service/          # Business logic + AiEngineClient + PipelineService
│       ├── security/         # JWT HS512, BCrypt-12, RBAC
│       ├── model/            # 13 JPA entities
│       └── resources/db/migration/  # Flyway V1–V14
├── ai-engine/                # FastAPI + LangGraph 7-stage pipeline agent
│   ├── main.py               # 8 endpoints + rate limiter
│   ├── agents/pipeline_agent.py    # 7-stage autonomous QA pipeline
│   ├── agents/real_agents_bridge.py # Bridge to D:\KumarFolder\Agents
│   └── tests/                # 60 unit tests
├── frontend/                 # React 18 + TypeScript + Tailwind
│   └── src/
│       ├── pages/            # 9 pages — Login, Dashboard, Projects, Pipeline, etc.
│       ├── components/       # Layout, SeverityBadge, StatusBadge
│       ├── hooks/            # useAuth (session-persistent), useWebSocket
│       └── services/         # api.ts + pipelineApi.ts
...
- Token persisted in localStorage (encrypted key `qaip_auth`), cleared on logout
